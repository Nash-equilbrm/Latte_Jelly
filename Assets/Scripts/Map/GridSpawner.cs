        ...
        internal void SpawnBlockSpawners(LevelConfig levelConfig)
        {
            if (_blockSpawners.Count < levelConfig.spawnerCount)
            {
                for (int i = _blockSpawners.Count + 1; i <= levelConfig.spawnerCount; ++i)
                {
                    ...
                    //var newSpawner = ObjectPooling.Instance.GetPool(Constants.SLOT_TAG).Get();
                    newSpawner.transform.SetParent(_spawnerHorizontalLayout.transform);
            ...
            for (int r = 0; r < levelConfig.rows; r++)
            {
                for (int c = 0; c < levelConfig.columns; c++)
                {
                    ...
                    Vector3 spawnPos = new Vector3(
                        (c - (float)levelConfig.columns / 2 + .5f) * Constants.CELL_SIZE.x, 
                        0,
                        -(r - (float)levelConfig.rows / 2 + .5f) * Constants.CELL_SIZE.y);
